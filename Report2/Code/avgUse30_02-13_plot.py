...
listDataLists = []
with open('avgUse30_data.tsv', 'rb') as tsvFile:
...
plt.plot(xdata, cigData, '-or', xdata, snuffData, '-sg', xdata, cigarData, '-xb', xdata, alcData, '-ok',
         xdata, mjData, '-sm', xdata, cocData, '-xr', xdata, heroinData, '-ob', xdata, halData, '-vm',
         xdata, inhData, '-xk', xdata, prData, '-om', xdata, tranqData, '-sr', xdata, stimData, '-xg',
         xdata, sedData, '-sb')

plt.title("Average 30 Day Usage From 2002-2013")
plt.xlabel("Year")
plt.ylabel("Average Number of Days Used In The Last 30)")
